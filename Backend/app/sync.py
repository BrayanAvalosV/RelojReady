...
def cargar_dataframe_en_mongodb(df):
    ...
    for documento in documentos:
        documento['estado'] = 'original'  # Agregar el atributo 'estado'
    dbregistro.tu_coleccion.insert_many(documentos)

# Conexión a MongoDB
def connect_to_mongodb():
    client = MongoClient(os.getenv("MONGO_URI", "mongodb://localhost:27017/"))
    db = client["mi_basedatos"]  # Cambia el nombre de la base de datos según lo necesites
    return db

# Guardar DataFrame en MongoDB
def guardar_dataframe_en_mongodb(df, db):
    coleccion = db["mi_coleccion"]  # Cambia el nombre de la colección según lo necesites
    documentos = df.to_dict(orient='records')
    for documento in documentos:
        documento['estado'] = 'original'  # Agregar el atributo 'estado'
    coleccion.insert_many(documentos)
    print(f"{len(documentos)} documentos insertados en MongoDB.")

# Extraer documentos de MongoDB
def extraer_documentos(db):
